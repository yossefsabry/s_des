...
split_array spliting(const char array[], int len) {
    ...
    int left_len = len / 2;
    char* l_arr = (char*)malloc(left_len + 1); // Allocate memory for left part
    if (l_arr == NULL) { // if error happend in malloc
        perror("Memory allocation failed");
        return sp_arr;
    }
    memcpy(l_arr, array, left_len); // copy a array to left
    l_arr[left_len] = '\0'; // Null-terminate l_arr (new line)

    // started with the right hand side
    int right_len = len - left_len;
    char* r_arr = (char*)malloc(right_len + 1); // Allocate memory for right part
    if (r_arr == NULL) {
        perror("Memory allocation failed");
        free(l_arr);
        return sp_arr;
    ...
    r_arr[right_len] = '\0'; // Null-terminate r_arr
